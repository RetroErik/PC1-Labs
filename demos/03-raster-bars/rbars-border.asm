...
; A proof-of-concept demonstrating per-scanline color changes
...
render_raster_bars:
    ...
    ; Loop through all visible scanlines
    xor cx, cx              ; CX = current scanline (0 to 199)
    
.scanline_loop:
    ; -----------------------------------------------------------------
    ; Wait for start of this scanline (horizontal retrace)
    ; Method: Wait for bit 0 to go high (retrace start)
    ; Note: This is a simple busy-wait. For more precision,
    ; one could count cycles, but this works well on the PC1.
    ; -----------------------------------------------------------------
    mov dx, PORT_STATUS
    
.wait_hretrace:
    in al, dx
    test al, 0x01           ; Check bit 0 (display enable/retrace)
    jz .wait_hretrace       ; Wait until retrace starts
    
    ; -----------------------------------------------------------------
    ; Calculate color for this scanline based on bar position
    ; -----------------------------------------------------------------
    mov ax, cx              ; AX = current scanline
    call get_bar_color      ; Returns color in AL (0-15)
    
    ; -----------------------------------------------------------------
    ; Write color to border register (instant effect)
    ; Port 0xD9 bits 0-3 control the border/background color
    ; -----------------------------------------------------------------
    mov dx, PORT_COLOR
    out dx, al
    
    ; -----------------------------------------------------------------
    ; Small delay to let the scanline complete before next iteration
    ; This prevents us from catching the same retrace multiple times.
    ; On the V40 @ 8 MHz, a few short jumps provide enough delay.
    ; -----------------------------------------------------------------
    jmp short $+2
    jmp short $+2
    jmp short $+2
    ...
    ; Reset border to black after last scanline (during VBLANK)
    xor al, al
    mov dx, PORT_COLOR
    out dx, al
...
enable_graphics_mode:
    ...
    ; BIOS Mode 4: CGA 320x200 graphics (sets up CRTC timing)
    mov ax, 0x0004
    int 0x10
    
    ; Register 0x67: Configuration Mode Register
    ; 0x18 = 8-bit bus mode (required for PC1), display timing bits set
    mov al, 0x67
    ...
    ; Register 0x65: Monitor Control Register
    ; 0x09 = 200 lines (bits 0-1=01), PAL timing (bit 3=1)
    mov al, 0x65
    out PORT_REG_ADDR, al
    jmp short $+2
    mov al, 0x09
