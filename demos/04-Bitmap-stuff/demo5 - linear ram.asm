...
;
; This copies the entire 160x200 image to VRAM, but shifted by (image_x, image_y).
; Areas outside the image bounds are filled with black (color 0).
;
; This is the "brute force" approach - simple but may cause some tearing
; if it extends past VBlank. We can optimize later if needed.
;
...
copy_image_at_position:
    ...
    mov bp, [image_y]           ; BP = Y offset (signed, in scanlines)
    
    ; Process each screen row (0-199)
    xor bx, bx                  ; BX = current screen row
    
.row_loop:
    ; Calculate source row in image buffer
    ; source_row = screen_row - image_y
    mov ax, bx
    sub ax, bp                  ; AX = screen_row - image_y
    
    ; Check if source row is valid (0-199)
    cmp ax, 0
    jl .fill_black_row          ; Source row < 0, fill with black
    cmp ax, SCREEN_HEIGHT
    jge .fill_black_row         ; Source row >= 200, fill with black
    
    ; Valid source row - calculate RAM buffer offset
    mov dx, BYTES_PER_LINE
    imul dx                     ; AX = source_row * 80
    mov si, image_buffer
    add si, ax                  ; SI = source pointer in RAM buffer
    
    ; Calculate VRAM destination offset for this screen row
    mov al, bl                  ; AL = screen row
    call calc_vram_offset       ; DI = VRAM offset
    
    ; Copy with X offset handling
    mov ax, [temp_x_bytes]      ; AX = X offset in bytes (signed)
    call copy_row_with_offset
    jmp .next_row
    
.fill_black_row:
    ; Fill this screen row with black
    mov al, bl
    call calc_vram_offset       ; DI = VRAM offset
