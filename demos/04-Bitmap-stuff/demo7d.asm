; ============================================================================
; DEMO7.ASM - Tall Image Viewport Scroller for Images taller than 200 rows
; Olivetti Prodest PC1 - V6355D 160x200x16 Hidden Graphics Mode
; Written for NASM - NEC V40 @ 8 MHz (80186 instruction set)
; By RetroErik - 2026
;
; ============================================================================
; SCROLLING TECHNIQUE COMPARISON: DEMO5 vs DEMO7
; ============================================================================
;
...
;
; Both demo5 and demo7 use SOFTWARE scrolling - copying pixels from RAM to
; VRAM each frame. The difference is in HOW they copy:
;
; +------------------+----------------------------------+-------------------------+
; | Aspect           | Demo5 (Row-by-Row)               | Demo7 (Bulk Block Copy) |
; +------------------+----------------------------------+-------------------------+
...
; +------------------+----------------------------------+-------------------------+
;
; Demo7 trades features (no X scroll) for speed (pure bulk copy).
; Both use REP MOVSW for the actual byte transfer.
;
; SOLUTION: Software viewport scrolling (demo7 approach)
...
;
; ============================================================================
; SPEED IMPROVEMENT: CIRCULAR BUFFER TECHNIQUE (not yet implemented)
; ============================================================================
;
; Current demo7 copies 16KB every frame. This can be improved 100x!
;
; The trick: Instead of copying all 200 rows, only copy the NEW rows that
; scroll into view (just 2 rows = 160 bytes), then use R12/R13 to shift
; the display start address within VRAM.
;
; How it works:
;   1. VRAM is treated as a circular buffer (wraps at 8KB boundary per bank)
;   2. When scrolling down: write 2 new rows at "bottom" of circular buffer
;   3. Use R12/R13 to change where the CRTC starts reading
;   4. CRTC wraps around automatically within the 8KB bank
;
; Example: Scroll down by 2 rows
;   - Current start address: 0x0000
;   - Write new rows 200-201 where rows 0-1 used to be
;   - Set R12/R13 to 0x0050 (80 bytes = 1 row offset)
;   - Display now shows rows 2-201 (rows 0-1 replaced with 200-201)
;
; Speed: 160 bytes/frame instead of 16,384 bytes/frame = ~100x faster!
;
...
; CRTC registers (unused in this demo - see demo7a.asm for R12/R13 usage)
...
msg_usage       db 'DEMO7 - Tall Image Viewport Scroller', 13, 10
